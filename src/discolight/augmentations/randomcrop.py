...
class RandomCrop(Augmentation):
    ...
    def params():
        ...
        return Params().add(
            "max_width", "Maximum width of cropped area (normalized)",
            BoundedNumber(float, 0, 1),
            0.7).add("max_height",
                     "Maximum height of cropped area (normalized)",
                     BoundedNumber(float, 0, 1),
                     0.7).add("min_width", "Minimum width of cropped area "
                              "(normalized)", BoundedNumber(float, 0, 1),
                              0.1).add(
                                  "min_height",
                                  "Minimum height of cropped area "
                                  "(normalized)", BoundedNumber(float, 0,
                                                                1), 0.1)
    ...
    def augment(self, img, bboxes, iteration=100):
        ...
        crop_width = random.randint(int(width * self.min_width),
                                    int(width * self.max_width))
        crop_height = random.randint(int(height * self.min_height),
                                     int(height * self.max_height))
